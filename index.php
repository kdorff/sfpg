		...
		function fillInfo(type, id)
		{
			if (!index || (type == 'img'))
			{
				...
				else if (type == 'img')
				{
					...
					info += '<strong>".sts(TEXT_IMAGE_DESCRIPTION)."</strong><br><div class=\"sfpg_info_text\">'+imgDescription[id] + '</div><br>';
					";
					if(PAYPAL_ENABLED)
					{
						echo"
		...
		function showImage(stage)
		{
			...
			if(stage==1)
			{
				if (preloadImg.complete)
				{
					...
					gebi('full_text').innerHTML = (imgDescription[index] != '' ? imgDescription[index] : imgName[index]);
					initDisplay();
					stage=2;
				}
			}
